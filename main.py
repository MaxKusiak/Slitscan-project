...
def choose_file():
    ...
    if file_path:
        ...
        tk.Label(other_params_frame, text=f"Початкова Y координата (від 1 до {height}):").grid(row=0, column=1, sticky="w")
        tk.Entry(other_params_frame, textvariable=start_y).grid(row=1, column=1, sticky="w", pady=(0,20))

        end_y.set(str(height))
        tk.Label(other_params_frame, text=f"Кінцева Y координата (від 1 до {height} і більша ніж початкова):").grid(row=2, column=1, sticky="w")
        tk.Entry(other_params_frame, textvariable=end_y).grid(row=3, column=1, sticky="w", pady=(0,20))
